...
import shef.ui.text.actions.DecoratedTextAction;
import shef.ui.text.actions.EnterKeyAction;
import shef.ui.text.actions.HTMLTextEditAction;
import shef.ui.text.actions.RemoveAction;
import shef.ui.text.actions.TabAction;
...
public class WysiwygHTMLEditorKit extends HTMLEditorKit {
	...
	private final ArrayList<ResizableView> monitoredViews = new ArrayList<>();
	private final MouseInputAdapter resizeHandler = new ResizeHandler();

	private final Map editorToActionsMap = new HashMap();
	private final KeyStroke tabBackwardKS = KeyStroke.getKeyStroke(KeyEvent.VK_TAB, InputEvent.SHIFT_DOWN_MASK);
	...
	public void install(JEditorPane ed) {
		...
		HTMLTextEditAction hteAction = new shef.ui.text.actions.PasteAction();
	...
	public void deinstall(JEditorPane ed) {
		...
		if (curAct instanceof shef.ui.text.actions.PasteAction) {
	...
	/**
	 * Factory to build views of the html elements. This simply extends the behavior of the default html factory to draw
	 * borderless tables, etc
	 */
	...
	private class ResizeHandler extends MouseInputAdapter {
		...
		/**
		 * Updates the list of monitored ResizeableViews. If they don't exist in the document, they're removed from the
		 * list.
		 *
		 * @param doc
		 */
	 ...
	 * <p>
	 * The default view for such {@link Element}s are big ugly blocky JTextFields, which doesn't really work well in
	 * practice and tends to confuse users. This view replaces the default and simply draws the elements as blocks that
